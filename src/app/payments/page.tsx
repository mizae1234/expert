...
import { CheckCircle2, XCircle, Clock, AlertTriangle, FileText, Save, ArrowLeft, ExternalLink, Eye } from 'lucide-react'
...
export default function PaymentsPage() {
  ...
  const handleApproveDetail = async (pr: any) => {
    try {
      setIsSaving(true)
      ...
      const updated = { ...pr, status: 'APPROVED', approvedBy: 'Manager', approvedAt: new Date().toISOString() }
      setRequests(prev => prev.map(r => r.id === pr.id ? updated : r))
  ...
  const handleRejectDetail = async (pr: any) => {
    ...
    try {
      ...
      const updated = { ...pr, status: 'REJECTED', rejectReason, approvedBy: 'Manager' }
      setRequests(prev => prev.map(r => r.id === pr.id ? updated : r))
      setSelectedPR(updated)
      showToast(`ปฏิเสธ ${pr.claimNo}`)
    } catch (err: any) {
      console.error(err)
      showToast('❌ เกิดข้อผิดพลาดในการปฏิเสธ')
    } finally {
      setIsSaving(false)
    }
  }

  const renderDetailView = (pr: any) => {
    let docUrl = null
    ...
    return (
      <div className="space-y-6 animate-fade-in pb-10">
        ...
        <div className="flex items-center gap-4 border-b pb-4">
          <Button variant="ghost" size="icon" onClick={() => setSelectedPR(null)} className="h-9 w-9 text-slate-500 hover:text-slate-800 bg-white shadow-sm border">
                ...
                {pr.status === 'PENDING_APPROVAL' ? (
                  <>
                    ...
                    <Button 
                      ...
                      onClick={() => handleApproveDetail(pr)}
                      disabled={isSaving}
                    >
                      ✓ อนุมัติการชำระเงิน
                    </Button>
  ...
  if (selectedPR) {
    return renderDetailView(selectedPR)
  }
